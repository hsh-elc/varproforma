package proforma.varproforma.fx.sample;

import java.io.IOException;
import java.util.stream.IntStream;

import javax.xml.bind.JAXBException;

import javafx.application.Application;
import javafx.stage.Stage;
import proforma.varproforma.CV;
import proforma.varproforma.V;
import proforma.varproforma.VarSpecNode;
import proforma.varproforma.VarSpecRoot;
import proforma.varproforma.Vp;

public class Sample4c extends Application {

	@Override
	public void start(Stage primaryStage) throws JAXBException, IOException {
		
		final int vpNum= Integer.parseInt(a1);
		
		Vp[] vps= (Vp[])IntStream.range(1, vpNum+1).mapToObj(i -> Vp.i(String.format("p%03d", i))).toArray(sz -> new Vp[sz]);

		VarSpecNode node= VarSpecRoot.build(vps);
		node= node.combineGroup();
		for (int i=1; i<=vpNum; i++) {
			node= node.range(1, vpNum*1000);
		}
		node= node.endCombineGroup();
		VarSpecRoot cvSpec= node.endBuild();
		
		Integer[] values= IntStream.range(1, vpNum+1).boxed().toArray(sz -> new Integer[sz]);
        V[] vs= new V[values.length];
        for (int i=0; i<vs.length; i++) {
            vs[i]= V.fromValue(values[i], vps[i]);
        }
        CV defaultValue= CV.fromValues((Object[])vs);

		Sample.start("Sample4c", cvSpec, defaultValue, primaryStage);
	}
	
	private static String a1;
	public static void main(String[] args) {
		
		a1= args[0];
		launch(args);
	}
}